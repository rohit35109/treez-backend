...
export class AppService {
  ...
  getTransactions(searchedText: string = "", status: string = ""): TransactionPayload[] {

    const statusFilter = status.split(",");
    let records = this.getRandomData();

    if (searchedText !== "") {
      records = records.filter((item) => item.customer.includes(searchedText) || item.external_id.includes(searchedText) ||
            item.gross_amount.toString().includes(searchedText) || item.source.includes(searchedText) ||
            item.swifter_id.includes(searchedText));
    }

    console.log(statusFilter.includes('initiated'));
    if (statusFilter && statusFilter.length > 0) {
      records = records.filter((item) => statusFilter.includes(item.status));
    }
  ...
  getRandomData(): TransactionPayload[] {
    ...
    const customerNames = ['Eren Akichi', 'Mikasa Ackerman', 'Armin Arlert', 'Levi Ackerman', 'Sasha Braus'];
